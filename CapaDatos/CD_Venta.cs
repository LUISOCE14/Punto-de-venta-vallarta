    ...
    public class CD_Venta
    {
        public int obtenerCorrelativo()
        {
            ...
            using (SqlConnection cconexion = new SqlConnection(Conexion.cadena))
            {
                ...
                try
                {
                    string query = "select max(idVenta) +1 from Ventas";
        ...
        public Venta obtenerVenta(int idventa)
        {
            ...
            using (SqlConnection cconexion = new SqlConnection(Conexion.cadena))
            {
                ...
                try
                {
                    ...
                    query.AppendLine("c.idCliente,c.Nombre,c.Telefono,c.Correo,c.Direccion,");
        ...
        public List<DetalleVenta> DetallesVenta(int idventa)
        {
            ...
            try
            {
                using (SqlConnection cconexion = new SqlConnection(Conexion.cadena))
                {
                    cconexion.Open();
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("select p.idProducto,p.NombreProducto,p.Modelo,dv.Cantidad,dv.PrecioProducto");
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            olista.Add(new DetalleVenta()
                            {
                                ...
                                PrecioVenta = Convert.ToDecimal(dr["PrecioProducto"])
                            });
                        }
                    }
